...
namespace mcx {
    ...
    public:
        ...
        __forceinline__ __device__ float exponential(float lambda, float bias = 0);
    };
    ...
    __forceinline__ __device__ float Random::exponential(float lambda, float bias) {
        return -logf(this->xorshift128p_nextf() + bias) / lambda;
    }
